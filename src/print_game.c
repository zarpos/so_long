...
/*   Updated: 2023/03/20 21:20:36 by drubio-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	all_floor(t_map *map)
{
	...
	while (map->map[y])
	{
		...
		while (map->map[y][x])
		{
			mlx_put_image_to_window(map->mlx, map->win, map->f, x * 56, y * 56);
			x++;
		}
		y++;
	}
...
{
	if (map->map[y][x] == '1')
		mlx_put_image_to_window(map->mlx, map->win, map->w, x * 56, y * 56);
	if (map->map[y][x] == 'P')
		mlx_put_image_to_window(map->mlx, map->win, map->p, x * 56, y * 56);
	if (map->map[y][x] == 'C')
